...
static CONST char Key1_STR[]="K1 is down\n\r";  //key 6 string
/*********************************************************************
 * EXTERNAL VARIABLES
 */
...
void OnBoard_KeyCallback ( uint8 keys, uint8 state )
{
  ...
  {
  	 static attHandleValueNoti_t temp;
	 bStatus_t error;
	temp.handle=0x0039;
    // Process SW1 here
    ...
    {
    	NPI_WriteTransport("S1 is down\n\r", 12);
	temp.len=12;
	temp.pValue=GATT_bm_alloc(SIMPLEPROFILE_CHAR7, ATT_HANDLE_VALUE_NOTI, 12, NULL);
	osal_memcpy(temp.pValue, Key1_STR, temp.len);
	SimpleProfile_SetParameter( SIMPLEPROFILE_CHAR7, temp.len, temp.pValue);
	error=GATT_Notification(0, &temp, FALSE);
    }
  }
